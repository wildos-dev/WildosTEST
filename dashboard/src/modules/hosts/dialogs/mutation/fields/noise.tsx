...
const NoiseErrorPopover: React.FC<{
    ...
    return (
        <Popover>
            <PopoverTrigger>
                <Icon name="MailWarning" className="p-0 size-5 text-destructive bg-primary-background" />
            </PopoverTrigger>
...
export const NoiseField = () => {
    ...
    const [forms, setForms] = React.useState<NoiseForm["noise"][number][]>(
    ...
    const enableForm = (index: number) => {
        ...
        if (index >= 0 && index < updatedForms.length) {
                        ...
                        {index === 0 && (
                            <FormLabel className="flex flex-row justify-between items-center">
                                ...
                                <div className="flex flex-row items-center gap-2">
                                    {errors.noise?.[index] && (
                                        <NoiseErrorPopover
                                            errors={errors.noise[index] as { packet?: FieldError; type?: FieldError; delay?: FieldError } | undefined}
                                        />
                                    )}
                                    {forms.length < 5 && (
                                        <Button
                                                ...
                                                >
                                                    <FormControl>
                                                        <SelectTrigger className="w-2/5">
                                                            <SelectValue placeholder={t("placeholders.noise-type")} />
                                                        </SelectTrigger>
                                                    </FormControl>
                                                    <SelectContent>
                                                        {noiseTypes.map(
                                            ...
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <Input
                                                        ...
                                                        placeholder={t("placeholders.packet")}
                                                        {...field}
                                                    />
                                                )}
                                            />
