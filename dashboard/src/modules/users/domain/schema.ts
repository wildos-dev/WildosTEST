...
export const UserSchema = z.object({
    username: z.string().min(1, { message: "validation.users.username_required" }),
    note: z.string().nullable(),
    data_limit: z
        .union([z.string().transform((str) => Number(str)), z.number()])
        .refine((val) => val >= 0, { message: "validation.minimum_zero" })
    ...
    usage_duration: z.number().max(9999 * 365 * 24 * 60 * 60, { message: "validation.usage_duration_max" })
        .nullable().optional(),
    ...
    service_ids: z
        ...
        .nonempty({ message: "validation.users.services_required" })
