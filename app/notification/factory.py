...
class AdminNotificationFactory(NotificationFactory):
    def create_notification(self, action: Enum, **kwargs: Any) -> AdminNotif:
        # For now, return a basic AdminNotif
        return AdminNotif()
...
class UserNotificationFactory(NotificationFactory):
    ...
    def create_notification(self, action: Enum, **kwargs: Any) -> UserNotification:
        if not isinstance(action, UserNotification.Action):
            raise ValueError(f"Invalid action type: {action}")
        
        notification_class = self.notification_classes.get(action)
        if not notification_class:
            raise ValueError(f"No notification class for action: {action}")
        
        # Extract user and by from kwargs
        user = kwargs.pop('user', None)
        by = kwargs.pop('by', None)
        
        if not user:
            raise ValueError("User is required for UserNotification")
...
class NotificationStrategy:
    ...
    def create_notification(
        ...
        if isinstance(action, UserNotification.Action):
            ...
            # For non-user actions, return a basic AdminNotif
