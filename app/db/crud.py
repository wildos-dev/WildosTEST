    ...
    for inb in inbounds:
        if inb.tag in current_tags:
            stmt = (
                ...
                .values(
                    protocol=json.loads(str(inb.config))["protocol"],  # type: ignore[arg-type]
                    config=inb.config,
                )
            )
            db.execute(stmt)
...
def get_hosts_for_user(session, user_id):
    # Fetch the user object with services loaded to avoid N+1 queries
    from sqlalchemy.orm import selectinload
    user = session.query(User).options(selectinload(User.services)).filter(User.id == user_id).one()
...
def update_host(db: Session, db_host: InboundHost, host: InboundHostModify):
    # SQLAlchemy model attribute assignments - type ignore for LSP compatibility
    db_host.remark = host.remark  # type: ignore[assignment]
    db_host.address = host.address  # type: ignore[assignment]
    db_host.uuid = host.uuid  # type: ignore[assignment]
    db_host.password = host.password  # type: ignore[assignment]
    db_host.host_network = host.network  # type: ignore[assignment]
    db_host.host_protocol = host.protocol  # type: ignore[assignment]
    db_host.port = host.port  # type: ignore[assignment]
    db_host.path = host.path  # type: ignore[assignment]
    db_host.sni = host.sni  # type: ignore[assignment]
    db_host.host = host.host  # type: ignore[assignment]
    db_host.security = host.security  # type: ignore[assignment]
    db_host.alpn = host.alpn.value  # type: ignore[assignment]
    db_host.fingerprint = host.fingerprint  # type: ignore[assignment]
    db_host.fragment = host.fragment.model_dump() if host.fragment else None  # type: ignore[assignment]
    setattr(db_host, 'mux_settings', host.mux_settings.model_dump() if host.mux_settings else None)
    db_host.is_disabled = host.is_disabled  # type: ignore[assignment]
    db_host.allowinsecure = host.allowinsecure  # type: ignore[assignment]
    setattr(db_host, 'udp_noises', [noise.model_dump() for noise in host.noise] if host.noise else None)
    db_host.header_type = host.header_type  # type: ignore[assignment]
    db_host.reality_public_key = host.reality_public_key  # type: ignore[assignment]
    db_host.reality_short_ids = host.reality_short_ids  # type: ignore[assignment]
    db_host.flow = host.flow  # type: ignore[assignment]
    db_host.shadowtls_version = host.shadowtls_version  # type: ignore[assignment]
    db_host.shadowsocks_method = host.shadowsocks_method  # type: ignore[assignment]
    setattr(db_host, 'splithttp_settings', host.splithttp_settings.model_dump() if host.splithttp_settings else None)
    db_host.early_data = host.early_data  # type: ignore[assignment]
    ...
    db_host.http_headers = host.http_headers  # type: ignore[assignment]
    db_host.mtu = host.mtu  # type: ignore[assignment]
    db_host.dns_servers = host.dns_servers  # type: ignore[assignment]
    db_host.allowed_ips = host.allowed_ips  # type: ignore[assignment]
    db_host.universal = host.universal  # type: ignore[assignment]
    db_host.services = (
        db.query(Service).filter(Service.id.in_(host.service_ids)).all()
    )
    db_host.weight = host.weight  # type: ignore[assignment]
    db.commit()
    db.refresh(db_host)
    return db_host
...
def get_users(
    ...
    if isinstance(is_active, bool):
        query = query.filter(User.is_active == is_active)  # type: ignore[arg-type]

    if isinstance(activated, bool):
        query = query.filter(User.activated == activated)  # type: ignore[arg-type]

    if isinstance(expired, bool):
        query = query.filter(User.expired == expired)  # type: ignore[arg-type]

    if isinstance(data_limit_reached, bool):
        query = query.filter(User.data_limit_reached == data_limit_reached)  # type: ignore[arg-type]
...
def get_total_usages(
    ...
    if not bool(getattr(admin, 'is_sudo', False)):
        query = (
            query.filter(
                Admin.id == admin.id,
            )
...
def get_user_usages(
    ...
    result = UserUsageSeriesResponse(
        username=str(getattr(db_user, 'username', '')), node_usages=[], total=0
    )

    for node_id, rows in usages.items():
        node_usages = UserNodeUsageSeries(
            node_id=node_id, node_name=str(node_id_names[node_id]), usages=[]
        )
        current = start.astimezone(timezone.utc).replace(
            minute=0, second=0, microsecond=0
        )
...
def get_users_count(
    ...
    if online is True:
        query = query.filter(
            User.online_at > (datetime.now(timezone.utc) - timedelta(seconds=30))
        )
    elif online is False:
        query = query.filter(
            User.online_at < (datetime.now(timezone.utc) - timedelta(seconds=30))
        )
    if expire_strategy is not None:
        query = query.filter(User.expire_strategy == expire_strategy)
...
def remove_user(db: Session, dbuser: User):
    setattr(dbuser, 'username', None)
    setattr(dbuser, 'removed', True)
    setattr(dbuser, 'activated', False)
...
def update_user(
    ...
    if modify.data_limit is not None:
        setattr(dbuser, 'data_limit', modify.data_limit or None)

    if modify.expire_strategy is not None:
        setattr(dbuser, 'expire_strategy', modify.expire_strategy or None)

        if modify.expire_strategy == UserExpireStrategy.FIXED_DATE:
            setattr(dbuser, 'usage_duration', None)
            setattr(dbuser, 'activation_deadline', None)
        elif modify.expire_strategy == UserExpireStrategy.START_ON_FIRST_USE:
            setattr(dbuser, 'expire_date', None)
    ...
    if modify.expire_date is not None:
        setattr(dbuser, 'expire_date', modify.expire_date or None)

    if modify.note is not None:
        setattr(dbuser, 'note', modify.note or None)

    if modify.data_limit_reset_strategy is not None:
        setattr(dbuser, 'data_limit_reset_strategy', modify.data_limit_reset_strategy)

    if modify.activation_deadline is not None:
        setattr(dbuser, 'activation_deadline', modify.activation_deadline)

    if modify.usage_duration is not None:
        setattr(dbuser, 'usage_duration', modify.usage_duration)
    ...
    setattr(dbuser, 'edit_at', datetime.now(timezone.utc))
...
def reset_user_data_usage(db: Session, dbuser: User):
    setattr(dbuser, 'traffic_reset_at', datetime.now(timezone.utc))

    setattr(dbuser, 'used_traffic', 0)
...
def revoke_user_sub(db: Session, dbuser: User):
    setattr(dbuser, 'key', secrets.token_hex(16))
    setattr(dbuser, 'sub_revoked_at', datetime.now(timezone.utc))
    db.commit()
    db.refresh(dbuser)
    return dbuser
...
def update_user_sub(db: Session, dbuser: User, user_agent: str):
    stmt = (
        ...
        .values(
            sub_updated_at=datetime.now(timezone.utc),
    ...
    for attribute in [
        ...
        if not isinstance(getattr(modifications, attribute), NoneType):
            ...
            if attribute == "hashed_password":
                setattr(dbadmin, 'password_reset_at', datetime.now(timezone.utc))
...
def partial_update_admin(
    ...
    if modified_admin.is_sudo is not None:
        setattr(dbadmin, 'is_sudo', modified_admin.is_sudo)
    if (
        modified_admin.password is not None
        and bool(getattr(dbadmin, 'hashed_password', None)) != modified_admin.hashed_password
    ):
        setattr(dbadmin, 'hashed_password', modified_admin.hashed_password)
        setattr(dbadmin, 'password_reset_at', datetime.now(timezone.utc))
...
def update_service(
    ...
    if modification.name is not None:
        setattr(db_service, 'name', modification.name)

    if getattr(modification, 'inbound_ids', None) is not None:
        setattr(db_service, 'inbounds', 
            db.query(Inbound)
            .filter(Inbound.id.in_(getattr(modification, 'inbound_ids', [])))
            .all()
        )
...
def update_node(db: Session, dbnode: Node, modify: NodeModify):
    if modify.name is not None:
        setattr(dbnode, 'name', modify.name)

    if modify.address is not None:
        setattr(dbnode, 'address', modify.address)

    if modify.port is not None:
        setattr(dbnode, 'port', modify.port)

    if modify.status is NodeStatus.disabled:
        setattr(dbnode, 'status', modify.status)
        setattr(dbnode, 'xray_version', None)
        setattr(dbnode, 'message', None)
    else:
        setattr(dbnode, 'status', NodeStatus.unhealthy)

    if modify.usage_coefficient:
        setattr(dbnode, 'usage_coefficient', modify.usage_coefficient)

    if modify.connection_backend:
        setattr(dbnode, 'connection_backend', modify.connection_backend)
...
def update_node_status(
    ...
    if db_node:
        setattr(db_node, 'status', status)
        if message:
            setattr(db_node, 'message', message)
        setattr(db_node, 'last_status_change', datetime.now(timezone.utc))
    db.commit()
...
def update_node_certificate(db: Session, node_id: int, cert_data: dict) -> bool:
    ...
    try:
        ...
        setattr(node, 'certificate', cert_data.get("certificate"))
        setattr(node, 'private_key', cert_data.get("private_key"))  # Should be encrypted in production
        setattr(node, 'cert_created_at', cert_data.get("created_at"))
        setattr(node, 'cert_expires_at', cert_data.get("expires_at"))
...
def get_node_certificate(db: Session, node_id: int) -> Optional[dict]:
    ...
    try:
        node = db.query(Node).filter(Node.id == node_id).first()
        if not node or not bool(getattr(node, 'certificate', None)):
            return None
...
def upsert_peak_event(
    ...
    """Create or update peak event with deduplication logic"""
    import json
    from app.db.models import NodePeakEvent
    ...
    try:
        ...
        db.add(db_peak_event)
        db.commit()
        db.refresh(db_peak_event)
        return db_peak_event
        
    except IntegrityError:
        # Record already exists with same dedupe_key and started_at
        db.rollback()
        
        # Find and update existing record
        existing = db.query(NodePeakEvent).filter(
            NodePeakEvent.node_id == node_id,
            NodePeakEvent.dedupe_key == dedupe_key,
            NodePeakEvent.started_at == started_at
        ).first()
        
        if existing:
            # Update resolved_at if provided
            if resolved_at:
                setattr(existing, 'resolved_at', resolved_at)
                db.commit()
                db.refresh(existing)
            return existing
        else:
            # Shouldn't happen, but handle gracefully
            raise ValueError("Peak event deduplication failed")
...
def get_peak_events(
    ...
    """Get peak events from database with filtering"""
    from app.db.models import NodePeakEvent
    
    query = db.query(NodePeakEvent).filter(NodePeakEvent.node_id == node_id)
    
    # Filter by timestamp
    if since_ms > 0:
        since_datetime = datetime.fromtimestamp(since_ms / 1000)
        query = query.filter(NodePeakEvent.started_at >= since_datetime)
    
    # Filter by category
    if category:
        query = query.filter(NodePeakEvent.category == category)
    
    # Order by most recent first and limit results
    query = query.order_by(NodePeakEvent.started_at.desc()).limit(limit)
...
def update_token_usage(db: Session, token_id: int) -> bool:
    ...
    try:
        ...
        if token:
            setattr(token, 'last_used', datetime.now(timezone.utc))
            setattr(token, 'usage_count', (getattr(token, 'usage_count', 0) or 0) + 1)
            db.commit()
            return True
        return False
    except Exception:
...
def cleanup_expired_tokens(db: Session) -> int:
    ...
    try:
        result = db.query(NodeToken).filter(
            NodeToken.expires_at < datetime.now(timezone.utc)
        ).delete()
