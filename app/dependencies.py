...
def get_admin(
    ...
    if dbadmin.password_reset_at is not None:
        created_at = payload.get("created_at")
        if not created_at or dbadmin.password_reset_at > created_at:
            return

    if not bool(dbadmin.is_sudo) and not bool(dbadmin.enabled):
        return
...
def get_subscription_user(
    ...
    if db_user and str(db_user.key) == str(key):
        return db_user
    else:
        raise HTTPException(status_code=404)
...
def get_user(
    ...
    db_user = crud.get_user(db, username)
    if not (
        admin.is_sudo or (db_user is not None and db_user.admin.username == admin.username)
    ):
...
node_authenticator = NodeAuthManager()
...
def get_node_auth(
    ...
    """Extract and validate node authentication token"""
    # Remove "Bearer " prefix if present
    if token.startswith("Bearer "):
        token = token[7:]
    
    if not node_authenticator.validate_node_token(token, node_id, db):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
