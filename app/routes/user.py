...
from app.security.guards import RequireAdminAccess
from app.models.admin import Admin
from app.middleware.validation import StrictUserCreateRequest  
...
def get_users(
    ...
    query = select(User).where(User.removed.is_(False))

    admin_filter = dbadmin if not admin.is_sudo else None
    ...
    for name in user_filters:
        ...
        if value is not None:
            if name == "username":
                if len(username) > 1:
                    query = query.where(User.username.in_(username))
                else:
                    query = query.where(
    ...
    if admin_filter:
        query = query.where(User.admin == admin_filter)
...
async def add_user(
    new_user: StrictUserCreateRequest,
    db: DBDep, 
    admin: Annotated[Admin, RequireAdminAccess("create", "users")],
    request: Request
):
    """
    Add a new user with enhanced security validation
    ...
    # Check admin permissions for user creation
    ...
    security_logger.log_security_event(
        ...
        details={
            ...
            "services_count": len(new_user.services) if new_user.services else 0
        },
        severity="INFO",
        ip_address=client_ip,
        user_id=admin.id
    ...
    try:
        # Convert strict request to UserCreate model
        from app.models.user import UserExpireStrategy
        
        # Determine expire strategy based on provided fields
        if new_user.expire_date:
            expire_strategy = UserExpireStrategy.FIXED_DATE
        elif new_user.usage_duration:
        ...
        user_create = UserCreate(
            ...
            usage_duration=new_user.usage_duration,
            ...
            online_at=None,
            service_ids=new_user.services  # Map 'services' to 'service_ids'
        )
        ...
        security_logger.log_security_event(
            ...
            details={
                ...
                "attempted_username": new_user.username,
                "created_by": admin.username
            },
            severity="WARNING",
            ip_address=client_ip,
...
async def delete_expired(
    ...
    for user in db_users:
        expire_date = getattr(user, 'expire_date', None)
    ...
    for db_user in expired_users:
        ...
        logger.info("User `%s` removed", getattr(db_user, 'username', 'unknown'))
...
async def modify_user(
    ...
    if (
        ...
        setattr(db_user, 'activated', db_user.is_active)
        db.commit()
...
def get_user_services(user: UserDep, db: DBDep, admin: AdminDep):
    ...
    if not admin.is_sudo and not admin.all_services_access:
        query = query.where(Service.id.in_(admin.service_ids))
...
async def reset_user_data_usage(
    ...
    if db_user.is_active and not was_active:
        ...
        setattr(db_user, 'activated', True)
        db.commit()
